...
class RNN_Relation(object):
	...
	def __init__(self, num_classes, word_dict_size, d1_dict_size, d2_dict_size, type_dict_size, sentMax, wv, w_emb_size=50, d1_emb_size=5, d2_emb_size=5, dep_emb_size=5, type_emb_size=5, filter_sizes=[3,4,5], num_filters=100, l2_reg_lambda = 0.0, pooling='max'):
...
#		W_wemb =    tf.Variable(tf.random_uniform([word_dict_size, w_emb_size], -1.0, +1.0))
		...
		X = tf.concat([emb0, emb3, emb4], 2) #, emb5])
...
#			l2_loss += tf.nn.l2_loss(b_c)
			conv = tf.nn.conv2d(X_expanded, W_c, strides=[1, 1, 1, 1], padding="VALID", name="conv")
     			h = tf.nn.relu(tf.nn.bias_add(conv, b_c), name="relu") 		#shape (?, 19, 1, 70)
			# Maxpooling over the outputs
			pooled = tf.nn.max_pool(h, ksize=[1,sentMax-filter_size+1,1,1], strides=[1, 1, 1, 1], padding='VALID', name="pool")
